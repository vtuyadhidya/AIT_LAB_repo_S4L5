...
def calculate_probabilities(current_city, visited, pheromone, distance_matrix):
    ...
    for city in range(num_cities):
        if city not in visited:
            prob = (pheromone[current_city][city] ** alpha) * ((1 / (distance_matrix[current_city][city] + epsilon)) ** beta)
    ...
    if total == 0:
        probabilities = np.array([1 if city not in visited else 0 for city in range(num_cities)], dtype=float)
        total = probabilities.sum()
...
for iteration in range(num_iterations):
    ...
    for path, distance in zip(all_paths, all_distances):
        for i in range(len(path) - 1):
            ...
            pheromone[path[i + 1]][path[i]] += pheromone_constant / distance
